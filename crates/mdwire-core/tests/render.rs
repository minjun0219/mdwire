...
fn emphasis_spanning_a_line_break_is_one_range() {
    // 실제 모양(LLM 산출물 85줄 중 78줄) — 여는 마커는 보통 자리, 닫는 마커는 다음 줄.
    assert_eq!(
        tg("이번 주 결론은 **배포를 금요일에\n하지 않는다** 이고, 예외는 없다."),
        "이번 주 결론은 <b>배포를 금요일에\n하지 않는다</b> 이고, 예외는 없다."
    );
    // 정규식 변환기가 뒤집던 인위적 모양 — 줄 첫머리의 `**` 는 닫기가 아니라 **열기**다.
    // 먼저 열린 `채널**` 은 짝 잃은 마커라 버리고 둘째 줄이 온전한 굵게다. CommonMark 도
    // 슬랙(실측 2026-09-22)도 이렇게 읽는다 — 다른 점은 짝 잃은 `**` 를 글자로 남기지
    // 않는 것뿐이다.
    assert_eq!(
...
/// **짝 잃은 `**` 는 버리고, 홑마커는 글자로 둔다.** 앞이 글자인 `**` 가 홀로 남을
/// 이유는 없다 — 닫으려던 마커고, 되돌리면 출력에 마커가 남는다. 홑마커는 다르다:
/// 각주(`참고*`)·글롭(`underfront.*`)·주석(`/* */`)으로 쓰이는 글자라 버리면 내용 손실이다.
/// 실측(LLM 산출물 430건)에서 사라진 별표는 전부 이쪽이었다.
#[test]
fn an_orphaned_double_marker_is_dropped_but_a_single_stays() {
    assert_eq!(tg("온다* 뒤에 더 있다"), "온다* 뒤에 더 있다");
    assert_eq!(tg("꼬리**"), "꼬리");
    assert_eq!(tg("underfront.* (4개), minjunkim.* (3개)"), "underfront.* (4개), minjunkim.* (3개)");
    // 추측으로 연 것은 뒤의 마커가 닫지 않는다 — 주석의 별표 둘 다 글자다. 그 뒤의
    // 진짜 기울임은 살아 있다.
...
/// 여기가 뒤집히면 그게 이 저장소가 존재하는 이유인 바로 그 고장이다 — 열기로 읽어
/// 먼저 열린 쪽을 물린다.
#[test]
